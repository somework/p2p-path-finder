...
final class PathSearchEngineAlgorithmStressTest extends TestCase
{
    /**
    ...
    public function test_adversarial_graph_long_linear_chain(): void
    {
        ...
        $pathFinder1 = new PathSearchEngine(
        ...
        $pathFinder2 = new PathSearchEngine(
    ...
    public function test_adversarial_graph_star_topology(): void
    {
        ...
        $shortestHops = min(array_map(static fn ($p) => $p->hops(), $paths));
    ...
    public function test_large_graph_scalability(): void
    {
        ...
        foreach ($nodes as $i => $iValue) {
            $from = $iValue;
            $to = $nodes[($i + 1) % $nodesCount];
            $orderBook->add(OrderFactory::buy($from, $to, '50.000', '150.000', '1.050', 3, 3));

            // Add some shortcuts
            if (0 === $i % 3 && $i + 5 < $nodesCount) {
                $shortcut = $nodes[$i + 5];
