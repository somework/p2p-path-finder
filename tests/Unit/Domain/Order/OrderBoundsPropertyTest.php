...
final class OrderBoundsPropertyTest extends TestCase
{
    ...
    public function test_min_is_always_less_than_or_equal_to_max(): void
    {
        ...
        for ($i = 0; $i < $limit; ++$i) {
            ...
            $min = self::money($currency, $this->formatUnits($minUnits, $scale), $scale);
            $max = self::money($currency, $this->formatUnits($maxUnits, $scale), $scale);
    ...
    public function test_contains_respects_boundaries_inclusively(): void
    {
        ...
        for ($i = 0; $i < $limit; ++$i) {
            ...
            if ($minUnits > 0) {
                $belowMin = self::money($currency, $this->formatUnits($minUnits - 1, $scale), $scale);
                self::assertFalse($bounds->contains($belowMin));
            }

            // Property: value strictly above max is not contained
            $aboveMax = self::money($currency, $this->formatUnits($maxUnits + 1, $scale), $scale);
    ...
    public function test_contains_includes_all_values_between_bounds(): void
    {
        ...
        for ($i = 0; $i < $limit; ++$i) {
            ...
            for ($j = 0; $j < 5; ++$j) {
                ...
                $mid = self::money($currency, $this->formatUnits($midUnits, $scale), $scale);
    ...
    public function test_currency_must_match_across_bounds(): void
    {
        ...
        for ($i = 0; $i < $limit; ++$i) {
            ...
            $min = self::money($currency1, '10.00', $scale);
            $max = self::money($currency1, '100.00', $scale);
            $bounds = OrderBounds::from($min, $max);

            // Property: Currency mismatch should throw on contains()
            $wrongCurrency = self::money($currency2, '50.00', $scale);
    ...
    public function test_bounds_creation_requires_currency_consistency(): void
    {
        ...
        for ($i = 0; $i < $limit; ++$i) {
            ...
            $min = self::money($currency1, '10.00', $scale);
            $max = self::money($currency2, '100.00', $scale);
    ...
    public function test_bounds_creation_requires_min_less_than_or_equal_max(): void
    {
        ...
        for ($i = 0; $i < $limit; ++$i) {
            ...
            $min = self::money($currency, $this->formatUnits($amount2, $scale), $scale);
            $max = self::money($currency, $this->formatUnits($amount1, $scale), $scale);
    ...
    public function test_clamp_always_returns_value_within_bounds(): void
    {
        ...
        for ($i = 0; $i < $limit; ++$i) {
            ...
            foreach ($testCases as $testUnits) {
                $testAmount = self::money($currency, $this->formatUnits($testUnits, $scale), $scale);
    ...
    public function test_clamp_is_idempotent(): void
    {
        ...
        for ($i = 0; $i < $limit; ++$i) {
            ...
            $amount = self::money($currency, $this->formatUnits($this->randomUnits($scale), $scale), $scale);
    ...
    public function test_different_scales_normalized_to_max_scale(): void
    {
        ...
        for ($i = 0; $i < $limit; ++$i) {
            ...
            $min = self::money($currency, '10.00', $scale1);
            $max = self::money($currency, '100.00', $scale2);
    ...
    public function test_contains_normalizes_scales_for_comparison(): void
    {
        ...
        for ($i = 0; $i < $limit; ++$i) {
            ...
            $min = self::money($currency, '10.00', $boundsScale);
            $max = self::money($currency, '100.00', $boundsScale);
            $bounds = OrderBounds::from($min, $max);

            // Test with different scale
            $testScale = $this->randomizer->getInt(0, 18);
            $testValue = self::money($currency, '50.00', $testScale);
    ...
    public function test_equal_bounds_form_single_point(): void
    {
        ...
        for ($i = 0; $i < $limit; ++$i) {
            ...
            $amount = self::money($currency, $this->formatUnits($units, $scale), $scale);
            ...
            if ($units > 0) {
                $below = self::money($currency, $this->formatUnits($units - 1, $scale), $scale);
                self::assertFalse($bounds->contains($below));
            }

            $above = self::money($currency, $this->formatUnits($units + 1, $scale), $scale);
