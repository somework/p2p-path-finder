...
final class GraphBuilderTest extends TestCase
{
    ...
    public function test_build_creates_sell_edges_for_each_order(): void
    {
        ...
        // Verify that edges are ordered by some deterministic criteria
        // The exact ordering logic is tested elsewhere, here we just ensure consistency
        self::assertCount(2, $edges);
    ...
    public function test_build_handles_empty_orders_array(): void
    {
        $graph = $this->exportGraph((new GraphBuilder())->build([]));
    ...
    public function test_build_creates_graph_with_single_order(): void
    {
        $order = $this->createOrder(OrderSide::BUY, 'BTC', 'USD', '0.100', '1.000', '30000');
        ...
        self::assertSame(['BTC', 'USD'], array_keys($graph));
        self::assertCount(1, $graph['BTC']['edges']);
        self::assertCount(0, $graph['USD']['edges']);

        $edge = $graph['BTC']['edges'][0];
        $this->assertEdgeBasics($edge, 'BTC', 'USD', OrderSide::BUY, $order);
        $this->assertEdgeCapacities($edge, 'BTC', '0.100', '1.000', 'USD', '3000.000', '30000.000');
    }

    public function test_build_handles_orders_creating_cycles(): void
    {
        $orders = [
            $this->createOrder(OrderSide::BUY, 'BTC', 'USD', '0.100', '1.000', '30000'),
            $this->createOrder(OrderSide::SELL, 'BTC', 'USD', '3000.000', '30000.000', '0.100'), // Reverse direction
        ];

        $graph = $this->exportGraph((new GraphBuilder())->build($orders));

        self::assertCount(2, $graph);
        self::assertCount(1, $graph['BTC']['edges']); // BUY order: BTC -> USD
        self::assertCount(1, $graph['USD']['edges']); // SELL order: USD -> BTC
    }

    public function test_build_handles_orders_with_zero_minimum_amount(): void
    {
        $order = $this->createOrder(OrderSide::BUY, 'BTC', 'USD', '0.000', '1.000', '30000');

        $graph = $this->exportGraph((new GraphBuilder())->build([$order]));

        $edge = $graph['BTC']['edges'][0];
        $this->assertEdgeBasics($edge, 'BTC', 'USD', OrderSide::BUY, $order);
        $this->assertEdgeCapacities($edge, 'BTC', '0.000', '1.000', 'USD', '0.000', '30000.000');
        self::assertSame([], $edge['segments']); // No fees, so no segments
    }

    public function test_build_handles_orders_with_very_small_maximum_amount(): void
    {
        $order = $this->createOrder(OrderSide::BUY, 'BTC', 'USD', '0.100', '0.101', '30000');

        $graph = $this->exportGraph((new GraphBuilder())->build([$order]));

        $edge = $graph['BTC']['edges'][0];
        $this->assertEdgeBasics($edge, 'BTC', 'USD', OrderSide::BUY, $order);
        $this->assertEdgeCapacities($edge, 'BTC', '0.100', '0.101', 'USD', '3000.000', '3030.000');
    }

    public function test_build_handles_orders_with_same_min_max_bounds(): void
    {
        $order = $this->createOrder(OrderSide::BUY, 'BTC', 'USD', '1.000', '1.000', '30000');

        $graph = $this->exportGraph((new GraphBuilder())->build([$order]));

        $edge = $graph['BTC']['edges'][0];
        $this->assertEdgeBasics($edge, 'BTC', 'USD', OrderSide::BUY, $order);
        $this->assertEdgeCapacities($edge, 'BTC', '1.000', '1.000', 'USD', '30000.000', '30000.000');
        self::assertSame([], $edge['segments']); // No fees, so no segments
    }

    public function test_build_handles_orders_with_large_amounts(): void
    {
        $order = $this->createOrder(OrderSide::BUY, 'BTC', 'USD', '1000.000', '10000.000', '30000');

        $graph = $this->exportGraph((new GraphBuilder())->build([$order]));

        $edge = $graph['BTC']['edges'][0];
        $this->assertEdgeBasics($edge, 'BTC', 'USD', OrderSide::BUY, $order);
        $this->assertEdgeCapacities($edge, 'BTC', '1000.000', '10000.000', 'USD', '30000000.000', '300000000.000');
    }

    public function test_build_handles_point_orders_with_fees(): void
    {
        // Point order (min = max = 0) with fees
        $order = $this->createOrder(
            OrderSide::BUY,
            'BTC',
            'USD',
            ...
            '0.000', // Point order
            '30000',
            $this->percentageFeePolicy('0.01') // 1% fee
        );
        ...
        $edge = $graph['BTC']['edges'][0];
        $this->assertEdgeBasics($edge, 'BTC', 'USD', OrderSide::BUY, $order);
        // Point orders with fees may or may not create segments depending on fee calculation
        // The important thing is that the edge is created correctly
        self::assertIsArray($edge['segments']);
    }

    public function test_build_groups_edges_by_origin_currency(): void
    {
        $orders = [
            $this->createOrder(OrderSide::BUY, 'BTC', 'USD', '0.100', '1.000', '30000'),
            $this->createOrder(OrderSide::BUY, 'BTC', 'EUR', '0.200', '0.800', '28000'),
            $this->createOrder(OrderSide::BUY, 'ETH', 'USD', '1.000', '5.000', '2000'),
        ];

        $graph = $this->exportGraph((new GraphBuilder())->build($orders));

        // BTC should have 2 edges (to USD and EUR)
        self::assertCount(2, $graph['BTC']['edges']);
        $btcQuotes = array_map(static fn ($edge) => $edge['to'], $graph['BTC']['edges']);
        self::assertEqualsCanonicalizing(['EUR', 'USD'], $btcQuotes);

        // ETH should have 1 edge (to USD)
        self::assertCount(1, $graph['ETH']['edges']);
        self::assertSame('USD', $graph['ETH']['edges'][0]['to']);

        // USD should have no edges (only destinations)
        self::assertCount(0, $graph['USD']['edges']);
    }

    public function test_build_sorts_currencies_alphabetically(): void
    {
        $orders = [
            $this->createOrder(OrderSide::BUY, 'ZEC', 'AAA', '1.000', '1.000', '1.000'),
            $this->createOrder(OrderSide::BUY, 'BTC', 'ZEC', '1.000', '1.000', '1.000'),
            $this->createOrder(OrderSide::BUY, 'AAA', 'BTC', '1.000', '1.000', '1.000'),
        ];

        $graph = $this->exportGraph((new GraphBuilder())->build($orders));

        // Should be sorted: AAA, BTC, ZEC
        self::assertSame(['AAA', 'BTC', 'ZEC'], array_keys($graph));
    }
